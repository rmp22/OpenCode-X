...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".ocx")
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".ocx", "tui.json"),
...
it.instance("loads .ocx/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".ocx", "tui.json"), JSON.stringify({ theme: "fallback" }))
