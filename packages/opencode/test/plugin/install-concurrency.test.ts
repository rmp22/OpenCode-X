...
describe("plugin.install.concurrent", () => {
  test("serializes concurrent server config updates across processes", async () => {
    ...
    const cfg = await read(path.join(tmp.path, ConfigPaths.PROJECT_DIRECTORY, "opencode.jsonc"))
  ...
  test("serializes concurrent server+tui config updates across processes", async () => {
    ...
    const server = await read(path.join(tmp.path, ConfigPaths.PROJECT_DIRECTORY, "opencode.jsonc"))
    const tui = await read(path.join(tmp.path, ConfigPaths.PROJECT_DIRECTORY, "tui.jsonc"))
  ...
  test("preserves updates when existing config uses .json", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ConfigPaths.PROJECT_DIRECTORY, "opencode.json")
    ...
    expect(await Filesystem.exists(path.join(tmp.path, ConfigPaths.PROJECT_DIRECTORY, "opencode.jsonc"))).toBe(false)
  }, 25_000)
})
