...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ConfigPaths.PROJECT_DIRECTORY, "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ConfigPaths.PROJECT_DIRECTORY, "themes", preloadedThemeFile)
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ConfigPaths.PROJECT_DIRECTORY, "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ConfigPaths.PROJECT_DIRECTORY, "themes", themeFile)
