...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ConfigPaths.PROJECT_DIRECTORY
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ConfigPaths.PROJECT_DIRECTORY, "themes")
    const dest_dir = meta.scope === "local" ? local_dir : path.join(ConfigPaths.globalDirectory(), "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ConfigPaths.PROJECT_DIRECTORY, "tui.json"),
  }
}
