...
describe("Config", () => {
  ...
  it.live("loads global, ancestor, and .ocx configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".ocx"), { recursive: true })
            await fs.mkdir(path.join(directory, ".ocx"), { recursive: true })
            await Promise.all([
              ...
              fs.writeFile(path.join(root, ".ocx", "opencode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".ocx", "opencode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".ocx")),
              AbsolutePath.make(path.join(directory, ".ocx")),
            ])
